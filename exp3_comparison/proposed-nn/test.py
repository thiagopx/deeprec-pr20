...
def solve(instance):
    ...
    if extra_args:
        ...
        t0 = time()
        solver.solve(compatibilities_k)
        opt_time = time() - t0
...
def test():
    ...
    # parser.add_argument(
    #     '-mi', '--matrix-id', action='store', dest='matrix_id', required=True, type=str,
    #     default=None, help='Identifier of the compatibility matrix.'
    # )
    parser.add_argument(
        '-s', '--solver', action='store', dest='solver', required=False, type=str,
        default='nn', help='Solver.'
    )
    ...
    # solver setup
    if args.solver == 'nn':
        solver = SolverNN(maximize=True, mode='repetitive', seed=seed)
        extra_args = False

    # global compatibility matrix
    result_glob = json.load(open('comparison/proposed/{}_matrix.json'.format(args.dataset), 'r'))
    ...
    # save directory
    suffix = '' if args.solver == 'conc' else '-nn'
    dir_name = 'comparison/proposed{}'.format(suffix)
