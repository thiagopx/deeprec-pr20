...
annotation = json.load(open('datasets/D1/mechanical/annotation.json'))
map_doc_category = {doc: category for category in annotation for doc in annotation[category]}
...
for dataset in ['D1', 'D2']:
    ...
    for run in results:
        ...
        lang = 'en_US'
        if dataset == 'D1':
            fname = 'datasets/D1/mechanical/{}/{}.jpg'.format(doc, doc)
            category = map_doc_category[doc]
            lang = 'pt_BR'
        else:
            fname = 'datasets/D2/integral/{}.TIF'.format(doc)
            category = None

        img = cv2.imread(fname)[..., :: -1]
        num_words = number_of_words(img, lang)
        print('dataset={} doc={} :: number of words={} accuracy={:.2f}'.format(dataset, doc, num_words, accuracy))
        records.append([doc, category, dataset, num_words, 100 * accuracy])
...
df = pd.DataFrame.from_records(records, columns=('doc', 'category', 'dataset', 'num_words', 'Accuracy (\\%)'))
df_D1 = df[df['dataset'] == 'D1']
df_D2 = df[df['dataset'] == 'D2']

print('dataset D1 - S-Marques')
print(df_D1)
print('dataset D2 - S-Isri-Ocr')
print(df_D2)

for category in df_D1['category'].unique():
    print(category)
    print(df_D1[df_D1['category'] == category])

print(df.groupby(['category']).mean())
# df.sort_values(by='Accuracy (\\%)', inplace=True)
# print(df[['doc', 'Accuracy (\\%)']])

os.makedirs('analysis/accuracy_D1/integral', exist_ok=True)
os.makedirs('analysis/accuracy_D2/integral', exist_ok=True)
for index, row in df.iterrows():
    ...
    # print(row, doc)
    if dataset == 'D1':
        source_img = 'datasets/D1/mechanical/{}/{}.jpg'.format(doc, doc)
        shutil.copy(source_img, 'analysis/accuracy_D1/integral/{}_{}_{}.jpg'.format(accuracy, num_words, doc))
    else:
        source_img = 'datasets/D2/integral/{}.TIF'.format(doc)
        shutil.copy(source_img, 'analysis/accuracy_D2/integral/{}_{}_{}.TIF'.format(accuracy, num_words, doc))
