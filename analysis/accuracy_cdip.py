...
# map_doc_textarea = {}
# for entry in bboxes:
#     doc = os.path.splitext(entry['name'].split('/')[-1])[0]
#     id_ = annotation[doc]['id']
#     image = cv2.imread('datasets/{}'.format(doc2image[doc]))
#     area = image.shape[0] * image.shape[1]
    
#     labels = [label for label in entry['labels'] if label['category'] == 'table']
#     area_bboxes = 0
#     for label in labels:
#         box = label['box2d']
#         area_bboxes += (box['x2'] - box['x1']) * (box['y2'] - box['y1'])
#     map_doc_textarea[doc] = area_bboxes / area
