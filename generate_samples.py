...
def init(args):
    ...
    for class_ in ['positives', 'negatives']:
        os.makedirs('{}/{}/train'.format(args.save_dir, class_), exist_ok=True)
        os.makedirs('{}/{}/val'.format(args.save_dir, class_), exist_ok=True)
...
def generate_samples(args, sample_size=(32, 32)):
    ...
    if glob.glob('{}/**/*.npy'.format(args.save_dir), recursive=True):
    # if glob.glob('{}/**/*.jpg'.format(args.save_dir), recursive=True):
                ...
                for (i, j), label in zip([pos_comb, neg_comb], ['positives', 'negatives']):
                    ...
                    for y in range(0, height - sample_size[0], args.stride):
                        ...
                        if not is_neutral(sample, args.neutral_thresh) and count_doc[label] < args.max_samples:
                            ...
                            # cv2.imwrite('{}/{}/{}/{}.jpg'.format(args.save_dir, label, mode, count[label]), noisy)
                            np.save('{}/{}/{}/{}.npy'.format(args.save_dir, label, mode, count[label]), noisy)
                            count_doc[label] += 1
                            count[label] += 1
...
def generate_files(args):
    ...
    docs_neg_train = glob.glob('{}/negatives/train/*.npy'.format(args.save_dir))
    docs_neg_val = glob.glob('{}/negatives/val/*.npy'.format(args.save_dir))
    docs_pos_train = glob.glob('{}/positives/train/*.npy'.format(args.save_dir))
    docs_pos_val = glob.glob('{}/positives/val/*.npy'.format(args.save_dir))
...
if __name__ == '__main__':
    ...
    args.save_dir = os.path.expanduser(args.save_dir) # allows to use '~' in path names
